...
(() => {
  ...
  try{const saved=localStorage.getItem('starfireDifficulty');if(AI_DIFFICULTIES[saved])selectedDifficulty=saved;}catch{}
  ...
  function createUnitEnhanced(type,team,q,r,extra={}){
    ...
    return {id:uid('u'),type,def,team,q,r,hp:base,maxHp:base,baseMaxHp:base,route:[],moveProgress:0,target:null,manualTarget:false,
  ...
  freshState=function(started=false,difficulty=selectedDifficulty){
    tiles=generateMap();const playerCity=createCity('player',3,7,true,'曙光城'),enemyCity=createCity('enemy',16,7,true,'灰烬要塞'),cfg=difficultyConfig(difficulty);
    const s={started,paused:false,gameOver:false,speed:1,simTime:0,pulseTimer:0,enemySpawnTimer:0,enemyThink:0,difficulty,
      ...
      cities:[playerCity,enemyCity],units:[],effects:[],logs:[],completed:new Set(),research:null,aiTech:true,selection:{kind:'city',id:playerCity.id},hovered:null,showIntel:false,keys:new Set(),score:0,era:0,overdriveGlobal:0,uiTimer:0,toastSeq:0,tutorialActive:false,tutorialRewarded:false,singularityCooldown:0,
      camera:{x:axialToWorld(5,7).x,y:axialToWorld(5,7).y,zoom:.86},screen:{w:1,h:1,dpr:1},lastFrame:performance.now(),acc:0};
    s.units.push(createUnit('warrior','player',4,7),createUnit('worker','player',3,8,{aiWorker:false}),createUnit('scout','player',4,6));
    const spots=[[15,7],[16,8],[15,6],[17,7],[16,6]];cfg.initialUnits.forEach((type,i)=>s.units.push(createUnit(type,'enemy',spots[i][0],spots[i][1],{aiOrder:'garrison'})));
  ...
  function renderSettings(){const cfg=difficultyConfig(),sel=$('difficultySelect'),hint=$('difficultyHint'),desc=$('introDifficultyDesc');if(sel)sel.value=cfg.key;if(hint)hint.textContent=`${cfg.icon}${cfg.label}：${cfg.desc}`;if(desc)desc.textContent=`${cfg.label}：${cfg.desc}`;document.querySelectorAll('[data-difficulty]').forEach(b=>b.classList.toggle('active',b.dataset.difficulty===cfg.key));updateAudioUI();}
  ...
  completeProduct=function(city,item){ORIGINAL.completeProduct(city,item);gameAudio.sfx(UNIT_DEFS[item.id]?'complete':'build',1.05);};
  ...
  // ===== 难度选择、重开、启动与 WASD =====
  function setDifficulty(key,announce=true){if(!AI_DIFFICULTIES[key])return;const old=state?.difficulty;selectedDifficulty=key;try{localStorage.setItem('starfireDifficulty',key);}catch{}if(state){if(!state.started){const oldSpeed=state.speed||1;state=freshState(false,key);state.speed=oldSpeed;resizeCanvases();state.lastYield=calculateYield();state.logs=[];addLog('🏠 曙光城开始自然产生基础资源。','good');addLog(`${difficultyConfig(key).icon} 已按「${difficultyConfig(key).label}」规则重置敌军开局。`,'warn');clampCamera();}else{state.enemyAI.difficulty=key;state.enemyAI.waveTimer=Math.min(state.enemyAI.waveTimer,difficultyConfig(key).waveInterval);state.difficulty=key;applyDifficultyScales(state,false);}}renderSettings();renderPanels();if(announce&&old!==key){const cfg=difficultyConfig(key);toast(`${cfg.icon} 敌军 AI 已切换为「${cfg.label}」，立即生效。`,key==='hard'?'danger':key==='easy'?'good':'warn');addLog(`🎚️ 敌军 AI 难度切换为 ${cfg.label}。`,key==='hard'?'danger':'warn');gameAudio.sfx(key==='hard'?'alarm':'confirm');}}
  ...
  resetGame=function(started=true){ORIGINAL.resetGame(started);state.difficulty=selectedDifficulty;state.enemyAI.difficulty=selectedDifficulty;state.enemyAI.waveTimer=difficultyConfig().initialWave;renderSettings();if(started){chooseEnemyResearch();gameAudio.sfx('confirm');}addLog(`${difficultyConfig().icon} 灰烬文明按「${difficultyConfig().label}」规则开始独立发展。`,'warn');};
  ...
  state=freshState(false,selectedDifficulty);resizeCanvases();state.lastYield=calculateYield();state.logs=[];addLog('🏠 曙光城开始自然产生基础资源。','good');addLog('⚙️ 左下角设置可调整难度、声音和音量。');addLog(`${difficultyConfig().icon} 灰烬文明将进行经济、科研、生产、扩张与波次进攻。`,'warn');renderPanels();clampCamera();renderSettings();updateAudioUI('声音待启动');
  ...
  window.__STARFIRE_DEBUG__={
    ...
    setDifficulty, setSpeed:v=>{state.speed=clamp(Number(v)||1,.1,10);}, fortifyPlayer:()=>{const c=state.cities.find(x=>x.team==='player'&&x.capital);c.maxHp=999999;c.hp=999999;}, audio:()=>({ready:gameAudio.ready,muted:gameAudio.muted,volume:gameAudio.volume,contextState:gameAudio.ctx?.state||'none',toneCount:gameAudio.toneCount,lastError:gameAudio.lastError,controls:{settings:!!$('audioDockVolume')}})
  };
