...
(() => {
  ...
  const AI_DIFFICULTIES = {
    ...
    medium:{key:'medium',label:'中等',icon:'⚔️',desc:'敌军拥有独立经济、科研、工人和生产队列，会建立第二城市并组织混编波次。',economy:.90,researchSpeed:.90,techCost:1.08,productionSpeed:.82,unitHp:.96,cityHp:1,damage:.90,attackInterval:1.06,armor:0,cityArmor:1,cityAttack:12,maxUnits:20,maxCities:2,initialWave:14,waveInterval:18,waveSize:4,waveGrowth:.55,defenseScan:5,eliteChance:.03,workerCharges:4,improvementsPerCity:3,expandAt:26,capitalShield:90,shieldRegen:1.1,repairRate:3.3,startTechs:[],adaptEvery:70,maxAdapt:2,starting:{food:88,production:82,science:38,gold:92,energy:26},initialUnits:['raider','enemyArcher']},
  ...
  function addPlayerOpeningUnits(s,slot,primary=false,faction='blue',teamKind='spark'){
    ...
    s.units.push(createUnit(worker,'player',clamp(spots[1][0],0,mapWidth()-1),clamp(spots[1][1],0,mapHeight()-1),{aiWorker:!primary,allyAI:!primary,faction,aiOrder:''}));
  ...
  freshState=function(started=false,difficulty=selectedDifficulty,mapMode=selectedMapMode,leftAI=selectedLeftAI,rightAI=selectedRightAI,playerSide=selectedPlayerSide,playerFaction=selectedPlayerFaction){
    ...
    const s={started,paused:false,gameOver:false,speed:1,simTime:0,pulseTimer:0,enemySpawnTimer:0,enemyThink:0,difficulty,
      resources:{food:92,production:72,science:42,gold:118,energy:32},lastYield:{food:0,production:0,science:0,gold:0,energy:0},enemyAI:makeEnemyAI(difficulty),allyAI:makeAllyAI(leftAI),
  ...
  resourcePulse=function(){
    ...
    if(!state.tutorialActive&&state.enemyAI){const ey=calculateEnemyYield(),ai=state.enemyAI,cfg=difficultyConfig();ai.lastYield=ey;for(const k of RESOURCE_KEYS)ai.resources[k]+=ey[k]||0;for(const c of state.cities)if(c.team==='enemy'&&c.hp>0&&ai.resources.food>30+c.population*7&&c.population<(cfg.key==='hard'?18:12)){ai.resources.food-=5+c.population;c.population++;}
      if(cfg.maxAdapt>0){const mark=Math.floor(state.simTime/cfg.adaptEvery);if(mark>ai.lastAdaptMark&&ai.adaptation<cfg.maxAdapt){ai.lastAdaptMark=mark;ai.adaptation++;addLog(`🧠 ${enemyFactionLabel()} AI 完成第 ${ai.adaptation} 次战术适应，生产与火力继续提升。`,'warn');if(cfg.key==='hard')toast(`☠️ 困难 AI 战术适应 ${ai.adaptation}/${cfg.maxAdapt}`,'danger');}}
    }
  ...
  function allyQueueProduct(city,id){if(!city?.allyAI||city.queue.length>=7||!hasProductUnlock(id)||!playerCanProduceUnit(id))return false;const d=productDef(id);if(!d||!allyPay(d.cost))return false;city.queue.push({qid:uid('aq'),id,progress:0,time:d.time,cost:deepCost(d.cost),ally:true});state.allyAI.plan=`${city.name} 正在生产 ${d.name}`;return true;}
  function enemyUnlocked(id){const d=productDef(id);return !!d&&enemyHasTech(d.tech);}
  function enemyQueueProduct(city,id){if(!city||city.team!=='enemy'||city.queue.length>=5||!enemyUnlocked(id))return false;const d=productDef(id);if(!d)return false;if(isBuildingProduct(id)&&(city.buildings.includes(id)||city.queue.some(x=>x.id===id)))return false;if(!enemyPay(d.cost))return false;city.queue.push({qid:uid('eq'),id,progress:0,time:d.time,cost:deepCost(d.cost),enemy:true});state.enemyAI.plan=`${city.name} 正在生产 ${d.name}`;return true;}
  ...
  updateCities=function(dt){
    for(const c of state.cities){if(c.hp<=0)continue;c.flash=Math.max(0,c.flash-dt*2.5);
      ...
      if(c.team==='enemy'&&!state.tutorialActive&&c.queue.length){const item=c.queue[0],cfg=difficultyConfig();let mult=cfg.productionSpeed*((c.buildings.includes('warFoundry')||c.buildings.includes('forge'))?1.28:1)*(1+state.enemyAI.adaptation*.055)*(state.enemyAI.lastStand?1.32:1);item.progress+=dt*mult;if(item.progress>=item.time){c.queue.shift();completeEnemyProduct(c,item);renderPanels();}}
  ...
  queueProduct=function(city,id){
    ...
    if(city?.team==='player'&&UNIT_DEFS[id]?.enemy){
      ...
      const def=productDef(id);if(!def)return;if(city.queue.length>=7){toast('建造队列已满。','warn');return;}
    ...
    }else ORIGINAL.queueProduct(city,id);
