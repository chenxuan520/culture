...
function renderCitySelection(c){
  ...
  if(c.team==='player'){
    ...
    html+=`</div><div class="sub">可生产项目 <span>玩家侧均在数秒内完成</span></div><div class="products">`;
    for(const id of PRODUCT_IDS){const d=productDef(id),av=productAvailability(c,id),isBuilding=!!BUILDING_DEFS[id];html+=`<button class="product" data-product="${id}" ${av.ok?'':'disabled'}><div class="pIcon">${d.icon}</div><div><b>${d.name} ${isBuilding?'· 建筑':'· 单位'}</b><p>${d.desc}</p><span class="small ${av.ok?'muted':'danger'}">${av.reason}</span></div><div class="cost"><span>⏱ ${d.time.toFixed(1)}s</span><span>${costText(d.cost)}</span></div></button>`;}
    html+=`</div>`;
...
function renderTileSelection(t){
  ...
  let html=`<div class="card"><div class="hero"><div class="heroIcon">${imp?.icon||res?.icon||terrain.icon}</div><div><h2>${imp?.name||res?.name||terrain.name}地块</h2><p>六边形坐标 ${t.q}, ${t.r}。${imp?imp.desc:res?'尚未开发的战略资源，可派工人自动建设。':'可根据地形部署对应采集设施。'}</p>${t.improvement?healthBar(t.improvement.hp,t.improvement.maxHp):''}</div></div><div class="badges">${tags}</div><div class="sub">每战略脉冲贡献 <span>1.0 模拟秒</span></div><div class="yields">`;
  for(const k of RESOURCE_KEYS)html+=`<div class="yield">${RESOURCE_META[k].icon} ${RESOURCE_META[k].name} <b>+${y[k]||0}</b></div>`;html+=`</div>`;
  if(res)html+=`<p class="desc">资源潜在产出：${yieldText(res.yield)}。只有建成设施后才会计入文明脉冲。</p>`;
