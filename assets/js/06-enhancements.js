...
(() => {
  ...
  function factionKey(obj){if(!obj)return teamFactions(selectedPlayerFaction)[0];if(obj.faction)return obj.faction;if(obj.team==='enemy')return teamFactions(opposingTeam(selectedPlayerFaction))[0];return obj.allyAI?teamFactions(selectedPlayerFaction)[1]:teamFactions(selectedPlayerFaction)[0];}
  ...
  freshState=function(started=false,difficulty=selectedDifficulty,mapMode=selectedMapMode,leftAI=selectedLeftAI,rightAI=selectedRightAI,playerSide=selectedPlayerSide,playerFaction=selectedPlayerFaction){
    ...
    const friendlyOrder=teamFactions(playerFaction),enemyOrder=teamFactions(opposingTeam(playerFaction));
    const playerCity=setFaction(createCity('player',playerSlots[0].q,playerSlots[0].r,true,playerFaction==='ash'?'灰烬要塞':'曙光城'),friendlyOrder[0]),enemyCity=setFaction(createCity('enemy',enemySlots[0].q,enemySlots[0].r,true,playerFaction==='ash'?'曙光城':'灰烬要塞'),enemyOrder[0]);
  ...
  updateCamera=function(dt){let dx=0,dy=0;if(state.keys.has('ArrowLeft')||state.keys.has('KeyA'))dx--;if(state.keys.has('ArrowRight')||state.keys.has('KeyD'))dx++;if(state.keys.has('ArrowUp')||state.keys.has('KeyW'))dy--;if(state.keys.has('ArrowDown')||state.keys.has('KeyS'))dy++;if(state.started&&!state.gameOver&&state.pointer){const edge=28;if(state.pointer.x<edge)dx--;else if(state.pointer.x>state.screen.w-edge)dx++;if(state.pointer.y<edge)dy--;else if(state.pointer.y>state.screen.h-edge)dy++;}if(dx||dy){const len=Math.hypot(dx,dy),speed=480/state.camera.zoom;state.camera.x+=dx/len*speed*dt;state.camera.y+=dy/len*speed*dt;clampCamera();}};

  // ===== 教程补充：WASD、声音与真实敌军 AI =====
  const viewStep=TUTORIAL_STEPS.find(x=>x.check==='view');if(viewStep){viewStep.task='把鼠标移到地图边缘滚动视野，或用 WASD/方向键移动视野；也可滚轮缩放、按住空格查看全军情报。';viewStep.body=`鼠标靠近地图边缘会自动平移视野；也可使用 <span class="key">W</span><span class="key">A</span><span class="key">S</span><span class="key">D</span> 或 <span class="key">↑</span><span class="key">↓</span><span class="key">←</span><span class="key">→</span> 平移；鼠标滚轮以指针位置为中心缩放；点击右下角战术雷达可快速跳转。<div class="keys"><span class="key">G</span> 打开上次生产基地 <span class="key">1-9/0</span> 城市生产快捷键 <span class="key">空格</span> 显示名称血条 <span class="key">P</span> 暂停/继续 <span class="key">F1</span> 打开教程</div>`;}
