...
(() => {
  ...
  const ORIGINAL = {
    ...
    updateCities, healNearby, autoAcquire, targetAttack, targetArmor, damageMultiplier,
    ...
    finishWorkerBuild, setUnitRoute, setLockedTarget, destroyTarget, endGame,
  ...
  let selectedDifficulty='medium',selectedMapMode='default',selectedLeftAI=0,selectedRightAI=1,selectedWorkerDefaultAI=false;
  try{
  ...
  function addPlayerOpeningUnits(s,slot,primary=false){
    const spots=primary?[[slot.q+1,slot.r],[slot.q,slot.r+1],[slot.q+1,slot.r-1]]:[[slot.q+1,slot.r],[slot.q,slot.r+1]];
    s.units.push(createUnit(primary?'warrior':'archer','player',clamp(spots[0][0],0,mapWidth()-1),clamp(spots[0][1],0,mapHeight()-1),{aiWorker:!primary,allyAI:!primary}));
    s.units.push(createUnit('worker','player',clamp(spots[1][0],0,mapWidth()-1),clamp(spots[1][1],0,mapHeight()-1),{aiWorker:!primary,allyAI:!primary}));
    if(primary)s.units.push(createUnit('scout','player',clamp(spots[2][0],0,mapWidth()-1),clamp(spots[2][1],0,mapHeight()-1),{aiWorker:false}));
  }
  function addEnemyOpeningUnits(s,slot,types){
  ...
  freshState=function(started=false,difficulty=selectedDifficulty,mapMode=selectedMapMode,leftAI=selectedLeftAI,rightAI=selectedRightAI){
    ...
    for(let i=1;i<playerSlots.length;i++){const c=createCity('player',playerSlots[i].q,playerSlots[i].r,false,`盟友前哨 ${i}`);c.allyAI=true;s.cities.push(c);}
    for(let i=1;i<enemySlots.length;i++)s.cities.push(createCity('enemy',enemySlots[i].q,enemySlots[i].r,false,ENEMY_CITY_NAMES[i-1]||`灰烬前哨 ${i}`));
    playerSlots.forEach((slot,i)=>addPlayerOpeningUnits(s,slot,i===0));
    enemySlots.forEach(slot=>addEnemyOpeningUnits(s,slot,cfg.initialUnits));
  ...
  function tileYieldForTeam(tile,team){
    const out={};if(!tile?.improvement||tile.improvement.team!==team)return out;const imp=IMPROVEMENTS[tile.improvement.type];if(!imp)return out;
  ...
  function spawnEnemyNearCity(city,type,forceElite=false){const def=UNIT_DEFS[type],t=findSpawnTile(city,def),cfg=difficultyConfig(),elite=!!def.combat&&(forceElite||Math.random()<cfg.eliteChance);const u=createUnit(type,'enemy',t.q,t.r,{aiOrder:def.combat?'garrison':'economy',elite,charges:def.worker?cfg.workerCharges:0});applyEnemyUnitScale(u,cfg,true);state.units.push(u);burst(t.q,t.r,elite?'#ffd166':'#ff6d7e',elite?18:10,.7);return u;}
  ...
  updateCities=function(dt){
    for(const c of state.cities){if(c.hp<=0)continue;c.flash=Math.max(0,c.flash-dt*2.5);
      if(c.team==='player'&&c.allyAI&&!c.queue.length){
        ...
        if(hasProductUnlock(order)&&canAfford(productDef(order).cost))queueProduct(c,order);
      }
      if(c.team==='player'&&c.queue.length){const item=c.queue[0];let mult=1;if(hasBuilding(c,'forge'))mult*=1.25;if(item.id==='kirov'&&hasBuilding(c,'skyDock'))mult*=1.3;item.progress+=dt*mult;if(item.progress>=item.time){c.queue.shift();completeProduct(c,item);renderPanels();}}
  ...
  function finishEnemyWorkerBuild(u){const w=u.work,t=tileAt(w.q,w.r);if(!t||t.improvement){u.work=null;return;}const d=IMPROVEMENTS[w.type];t.improvement={type:w.type,team:'enemy',hp:d.hp,maxHp:d.hp};u.charges--;u.work=null;addLog(`⚠️ 灰烬工蜂完成 ${d.name}，敌方经济产出提高。`,'warn');burst(t.q,t.r,'#ff7b8a',12,.7);if(u.charges<=0){state.units=state.units.filter(x=>x.id!==u.id);if(state.selection?.id===u.id)state.selection=null;}}
  ...
  function foundEnemyCity(u){const t=tileAt(u.q,u.r),cfg=difficultyConfig();if(!t||!isLand(t)||cityAt(t.q,t.r))return false;const idx=state.cities.filter(c=>c.team==='enemy').length-1,c=createCity('enemy',t.q,t.r,false,ENEMY_CITY_NAMES[Math.min(idx,ENEMY_CITY_NAMES.length-1)]||`灰烬前哨 ${idx+1}`);state.cities.push(c);applyEnemyCityScale(c,cfg,true);state.units=state.units.filter(x=>x.id!==u.id);state.enemyAI.resources.production+=35;state.enemyAI.resources.gold+=20;state.enemyAI.plan=`${c.name} 开始建设`;addLog(`☠️ 灰烬播种舰建立了新城市「${c.name}」。`,'danger');toast(`⚠️ 敌军扩张：${c.name} 已建立！`,'danger');burst(c.q,c.r,'#ff6d7e',25,1.2);gameAudio.sfx('alarm');return true;}
  ...
  function renderSettings(){const cfg=difficultyConfig(),sel=$('difficultySelect'),hint=$('difficultyHint'),desc=$('introDifficultyDesc'),mapSel=$('mapModeSelect'),leftSel=$('leftAISlots'),rightSel=$('rightAISlots'),workerSel=$('workerDefaultAI'),skirmish=$('skirmishHint');if(sel)sel.value=cfg.key;if(hint)hint.textContent=`${cfg.icon}${cfg.label}：${cfg.desc}`;if(desc)desc.textContent=`${cfg.label}：${cfg.desc}`;if(mapSel)mapSel.value=selectedMapMode;if(leftSel)leftSel.value=String(selectedLeftAI);if(rightSel)rightSel.value=String(selectedRightAI);if(workerSel)workerSel.value=selectedWorkerDefaultAI?'on':'off';if(skirmish)skirmish.textContent=`${selectedMapMode==='random'?'随机地图':'默认地图'} · ${1+selectedLeftAI}v${selectedRightAI} · 新工人${selectedWorkerDefaultAI?'自动 AI':'手动 AI'}${selectedMapMode==='random'&&state?.mapSize?` · ${state.mapSize.width}×${state.mapSize.height}`:''}`;document.querySelectorAll('[data-difficulty]').forEach(b=>b.classList.toggle('active',b.dataset.difficulty===cfg.key));updateAudioUI();}
  renderPanels=function(){ORIGINAL.renderPanels();renderSettings();};

  // ===== 画面：敌军设施产出、生产队列、精英标记与攻击路线 =====
  drawResourcesAndImprovements=function(){for(const t of tiles.values()){if(!t.resource&&!t.improvement&&!t.ruin)continue;const p=axialToWorld(t.q,t.r);if(t.ruin){ctx.save();ctx.translate(p.x,p.y);ctx.rotate(state.simTime*.2);ctx.shadowBlur=14;ctx.shadowColor='#c997ff';ctx.font='21px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('✨',0,0);ctx.restore();}if(t.resource){ctx.beginPath();ctx.arc(p.x-13,p.y-10,12,0,Math.PI*2);ctx.fillStyle='rgba(3,10,18,.72)';ctx.fill();ctx.strokeStyle='rgba(255,209,102,.55)';ctx.lineWidth=1/state.camera.zoom;ctx.stroke();ctx.font='16px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(RESOURCE_DEFS[t.resource].icon,p.x-13,p.y-10);}if(t.improvement){const imp=IMPROVEMENTS[t.improvement.type],player=t.improvement.team==='player';ctx.save();ctx.shadowBlur=state.selection?.q===t.q&&state.selection?.r===t.r?14:5;ctx.shadowColor=player?'#66e7a7':'#ff6d7e';ctx.beginPath();ctx.arc(p.x+5,p.y+5,17,0,Math.PI*2);ctx.fillStyle='rgba(6,19,28,.9)';ctx.fill();ctx.lineWidth=2/state.camera.zoom;ctx.strokeStyle=player?'#66e7a7':'#ff6d7e';ctx.stroke();ctx.shadowBlur=0;ctx.font='21px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(imp.icon,p.x+5,p.y+5);ctx.restore();const y=tileYieldForTeam(t,t.improvement.team);ctx.font='bold 8px system-ui';ctx.textAlign='center';ctx.fillStyle=player?'#bff7d7':'#ffbac3';ctx.fillText(yieldText(y),p.x,p.y+29);if(t.improvement.hp<t.improvement.maxHp)drawWorldHealth(p.x-20,p.y+33,40,4,t.improvement.hp/t.improvement.maxHp,player?'#66e7a7':'#ff6d7e');}}};
  drawCities=function(){for(const c of state.cities){if(c.hp<=0)continue;const p=axialToWorld(c.q,c.r),player=c.team==='player',selected=state.selection?.kind==='city'&&state.selection.id===c.id;ctx.save();ctx.shadowBlur=selected?24:10;ctx.shadowColor=player?'#59dcff':'#ff6d7e';ctx.beginPath();ctx.arc(p.x,p.y,31,0,Math.PI*2);ctx.fillStyle=player?'rgba(16,65,86,.96)':'rgba(79,27,38,.96)';ctx.fill();ctx.lineWidth=(selected?4:2.5)/state.camera.zoom;ctx.strokeStyle=player?'#71e2ff':'#ff7889';ctx.stroke();ctx.beginPath();for(let i=0;i<8;i++){const a=i*Math.PI/4,r=i%2?27:34,xx=p.x+Math.cos(a)*r,yy=p.y+Math.sin(a)*r;i?ctx.lineTo(xx,yy):ctx.moveTo(xx,yy);}ctx.closePath();ctx.lineWidth=1.2/state.camera.zoom;ctx.strokeStyle=player?'rgba(173,241,255,.55)':'rgba(255,174,184,.5)';ctx.stroke();ctx.shadowBlur=0;if(c.flash>0){ctx.beginPath();ctx.arc(p.x,p.y,36+c.flash*7,0,Math.PI*2);ctx.strokeStyle=`rgba(255,255,255,${c.flash*.7})`;ctx.lineWidth=3/state.camera.zoom;ctx.stroke();}ctx.font=`${c.capital?31:27}px system-ui`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(player?(c.capital?'🏠':'🏙️'):(c.capital?'🏰':'🏯'),p.x,p.y-2);ctx.restore();if(c.shield>0){ctx.beginPath();ctx.arc(p.x,p.y,39,0,Math.PI*2);ctx.strokeStyle=player?'rgba(89,220,255,.55)':'rgba(201,151,255,.65)';ctx.lineWidth=2.2/state.camera.zoom;ctx.setLineDash([5,4]);ctx.lineDashOffset=-state.simTime*12;ctx.stroke();ctx.setLineDash([]);}drawWorldHealth(p.x-30,p.y+38,60,6,c.hp/c.maxHp,player?'#66e7a7':'#ff6d7e');ctx.font='bold 10px system-ui';ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillStyle='#edf8ff';ctx.fillText(c.name,p.x,p.y-38);if(c.queue.length){const item=c.queue[0],d=productDef(item.id);ctx.font='13px system-ui';ctx.fillStyle=player?'#edf8ff':'#ffc0c8';ctx.fillText(`${d.icon} ${Math.max(0,item.time-item.progress).toFixed(1)}s`,p.x,p.y+57);}}};
  ...
  drawUnits=function(){for(const u of state.units){if(u.hp<=0)continue;let p=unitDrawPos(u),off=unitStackOffset(u);p={x:p.x+off.x,y:p.y+off.y};const player=u.team==='player',selected=state.selection?.kind==='unit'&&state.selection.id===u.id,r=u.type==='kirov'||u.type==='enemyTitan'?23:18;ctx.save();ctx.shadowBlur=selected?23:u.combatGlow>0?15:u.elite?13:6;ctx.shadowColor=u.elite?'#ffd166':player?'#59dcff':'#ff6d7e';ctx.beginPath();ctx.arc(p.x,p.y,r,0,Math.PI*2);ctx.fillStyle=player?'rgba(12,49,72,.96)':'rgba(73,22,35,.96)';ctx.fill();ctx.lineWidth=(selected?3.4:u.elite?2.8:2)/state.camera.zoom;ctx.strokeStyle=u.elite?'#ffd166':player?'#72e4ff':'#ff7587';ctx.stroke();ctx.shadowBlur=0;if(u.overdrive>0){ctx.beginPath();ctx.arc(p.x,p.y,r+5+Math.sin(state.simTime*10)*2,0,Math.PI*2);ctx.strokeStyle='rgba(255,209,102,.8)';ctx.lineWidth=2/state.camera.zoom;ctx.stroke();}ctx.font=`${u.type==='kirov'||u.type==='enemyTitan'?27:22}px system-ui`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(u.def.icon,p.x,p.y);ctx.restore();if((u.type==='worker'||u.type==='enemyWorker')&&u.aiWorker){ctx.font='10px system-ui';ctx.fillText('🤖',p.x+r-2,p.y-r+2);}if(u.target){ctx.font='9px system-ui';ctx.fillText(u.team==='enemy'?'⚠️':'🎯',p.x-r+1,p.y-r+1);}if(u.elite){ctx.font='10px system-ui';ctx.fillText('⭐',p.x+r-1,p.y-r+1);}if(u.def.combat&&u.combatGlow>0){let interval=(u.def.interval||1)*(u.overdrive>0?.72:1);if(u.team==='enemy')interval*=difficultyConfig().attackInterval;const pct=clamp(u.attackTimer/interval,0,1);ctx.beginPath();ctx.arc(p.x,p.y,r+7,-Math.PI/2,-Math.PI/2+pct*Math.PI*2);ctx.strokeStyle=player?'#ffd166':'#ff6d7e';ctx.lineWidth=2.5/state.camera.zoom;ctx.stroke();}const show=state.showIntel||selected||u.combatGlow>0||u.hp<u.maxHp;if(show){drawWorldHealth(p.x-23,p.y+r+6,46,5,u.hp/u.maxHp,player?'#66e7a7':'#ff6d7e');ctx.font='bold 9px system-ui';ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillStyle='#f0f9ff';ctx.fillText((u.elite?'⭐':'')+u.def.name,p.x,p.y-r-7);}if(u.work?.building){const pct=u.work.progress/u.work.time;ctx.beginPath();ctx.arc(p.x,p.y,r+10,-Math.PI/2,-Math.PI/2+pct*Math.PI*2);ctx.strokeStyle=player?'#66e7a7':'#ff6d7e';ctx.lineWidth=3/state.camera.zoom;ctx.stroke();}}};
  drawMinimap=function(){ORIGINAL.drawMinimap();const tr=mini._mapTransform;if(!tr)return;const dpr=mini._dpr||1;mctx.setTransform(dpr,0,0,dpr,0,0);mctx.fillStyle='#ff6d7e';for(const t of tiles.values())if(t.improvement?.team==='enemy'){const p=axialToWorld(t.q,t.r);mctx.fillRect(tr.mx(p.x)-1.5,tr.my(p.y)-1.5,3,3);}};
  ...
  finishWorkerBuild=function(unit){ORIGINAL.finishWorkerBuild(unit);gameAudio.sfx('build',.9);};
  ...
  // ===== 难度选择、重开、启动与 WASD =====
  function restartWithSelectedConfig(started=false){const oldSpeed=state?.speed||1;state=freshState(started,selectedDifficulty,selectedMapMode,selectedLeftAI,selectedRightAI);state.speed=oldSpeed;resizeCanvases();state.lastYield=calculateYield();state.logs=[];addLog('🏠 曙光城开始自然产生基础资源。','good');addLog(`${selectedMapMode==='random'?'🎲 随机地图':'⬡ 默认地图'} · ${1+selectedLeftAI}v${selectedRightAI} 对战配置已载入。`,'good');addLog(`${difficultyConfig().icon} 灰烬文明按「${difficultyConfig().label}」规则开始独立发展。`,'warn');clampCamera();renderPanels();renderSettings();}
  ...
  const viewStep=TUTORIAL_STEPS.find(x=>x.check==='view');if(viewStep){viewStep.task='按住鼠标左键或中键拖动地图，或用 WASD/方向键移动视野；也可滚轮缩放、按住空格查看全军情报。';viewStep.body=`在地图空白处或单位上 <b>按住鼠标左键（或中键）并拖动</b>，地图会跟随手势平移；短按左键仍然正常选择目标。也可使用 <span class="key">W</span><span class="key">A</span><span class="key">S</span><span class="key">D</span> 或 <span class="key">↑</span><span class="key">↓</span><span class="key">←</span><span class="key">→</span> 平移；鼠标滚轮以指针位置为中心缩放；点击右下角战术雷达可快速跳转。<div class="keys"><span class="key">空格</span> 按住显示所有单位名称和血条 <span class="key">P</span> 暂停/继续 <span class="key">F1</span> 打开教程 <span class="key">C</span> 随机清除一半敌军</div>`;}
  TUTORIAL_STEPS.splice(TUTORIAL_STEPS.length-1,0,{icon:'⚙️',title:'开局设置、声音与敌军 AI',subtitle:'左下角齿轮控制地图、对战规模、难度和声音。',target:'#gameSettings',place:'left',task:'调整一次地图模式、对战人数、音量、静音开关或敌军 AI 难度。',check:'settings',body:`左下角 <b>设置</b> 里可以调整地图、对战规模、敌军难度、声音和新工人默认 AI。<br><br><em>默认地图</em>是稳定教学地图；<em>随机地图</em>会重掷地图面积、地形和资源。左侧 AI 是盟友，右侧 AI 是敌人，最多可以组成 <b>3v3</b>。新工人默认 AI 开启后，城市生产出的工人会自动找资源地块建设。<br><br>敌军难度有三档：简单发展较慢；中等会建第二城市并持续进攻；困难会快速科研、扩张和生产精英军团。`,tip:'这些设置会影响下一局；未开始游戏时切换会立即重置预览开局。'});
  ...
  state=freshState(false,selectedDifficulty,selectedMapMode,selectedLeftAI,selectedRightAI);resizeCanvases();state.lastYield=calculateYield();state.logs=[];addLog('🏠 曙光城开始自然产生基础资源。','good');addLog('⚙️ 左下角设置可调整地图、对战、难度、声音和音量。');addLog(`${difficultyConfig().icon} 灰烬文明将进行经济、科研、生产、扩张与波次进攻。`,'warn');renderPanels();clampCamera();renderSettings();updateAudioUI('声音待启动');
  ...
  window.__STARFIRE_DEBUG__={
    ...
    setDifficulty,setMapMode,setSkirmishSlots,setWorkerDefaultAI, setSpeed:v=>{state.speed=clamp(Number(v)||1,.1,10);}, fortifyPlayer:()=>{const c=state.cities.find(x=>x.team==='player'&&x.capital);c.maxHp=999999;c.hp=999999;}, audio:()=>({ready:gameAudio.ready,muted:gameAudio.muted,volume:gameAudio.volume,contextState:gameAudio.ctx?.state||'none',toneCount:gameAudio.toneCount,lastError:gameAudio.lastError,controls:{settings:!!$('audioDockVolume')}})
  };
