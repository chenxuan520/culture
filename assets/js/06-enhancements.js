...
(() => {
  ...
  window.__STARFIRE_DEBUG__={
    ...
    setDifficulty,setMapMode,setSkirmishSlots,setWorkerDefaultAI,setSettlerAutoFound, allyQueueProduct, calculateAllyYield, updateEnemyStrategicAI, setSpeed:v=>{state.speed=clamp(Number(v)||1,.1,10);}, fortifyPlayer:()=>{const c=state.cities.find(x=>x.team==='player'&&x.capital);c.maxHp=999999;c.hp=999999;}, audio:()=>({ready:gameAudio.ready,muted:gameAudio.muted,volume:gameAudio.volume,contextState:gameAudio.ctx?.state||'none',toneCount:gameAudio.toneCount,lastError:gameAudio.lastError,controls:{settings:!!$('audioDockVolume')}})
  };
