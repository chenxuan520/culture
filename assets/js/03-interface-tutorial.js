...
function renderCitySelection(c){
  ...
  if(c.team==='player'){
    ...
    for(const [idx,id] of PRODUCT_IDS.entries()){const d=productDef(id),av=productAvailability(c,id),isBuilding=!!BUILDING_DEFS[id],hot=idx<10?['1','2','3','4','5','6','7','8','9','0'][idx]:'';html+=`<button class="product" data-product="${id}" ${av.ok?'':'disabled'}><div class="pIcon">${hot||d.icon}</div><div><b>${d.name} ${isBuilding?'· 建筑':'· 单位'}</b><p>${d.desc}</p><span class="small ${av.ok?'muted':'danger'}">${hot?`快捷键 ${hot} · `:''}${av.reason}</span></div><div class="cost"><span>⏱ ${d.time.toFixed(1)}s</span><span>${costText(d.cost)}</span></div></button>`;}
    html+=`</div>`;
