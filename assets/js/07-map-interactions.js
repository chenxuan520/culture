...
(() => {
  ...
  function beginMapDrag(e){
    ...
    dragState.startCamX=state.camera.x;dragState.startCamY=state.camera.y;
    dragState.currentCamX=state.camera.x;dragState.currentCamY=state.camera.y;
    dragState.mode=rightDrag?'pan':'box';
  ...
  function moveMapDrag(e){
    ...
    if(!dragState.moved){
      ...
      if(dragState.mode==='box')canvas.classList.add('isBoxSelecting');
      const tip=$('tooltip');if(tip)tip.style.display='none';
      if(tutorial?.active){tutorial.flags.viewAction=true;updateTutorialTask();}
    }
    if(dragState.mode==='pan'){
      state.camera.x=dragState.startCamX-dx/state.camera.zoom;
      state.camera.y=dragState.startCamY-dy/state.camera.zoom;
      dragState.currentCamX=state.camera.x;dragState.currentCamY=state.camera.y;
      clampCamera();
    }
  ...
  function finishMapDrag(e){
    if(dragState.pointerId!==e.pointerId)return;
    if(dragState.moved&&dragState.mode==='box'){
    ...
    if(dragState.moved&&dragState.mode==='pan')state.rightPanSuppressUntil=performance.now()+250;
  ...
  canvas.addEventListener('auxclick',e=>{if(e.button===1||e.button===2)e.preventDefault();});

  const oldDrawMap=drawMap;
  drawMap=function(){
    oldDrawMap();
    if(!dragState.moved||dragState.mode!=='box')return;
