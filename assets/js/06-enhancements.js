...
(() => {
  ...
  let selectedDifficulty='medium',selectedMapMode='default',selectedLeftAI=0,selectedRightAI=1,selectedWorkerDefaultAI=false,selectedSettlerAutoFound=false;
  try{
  ...
  function renderSettings(){const cfg=difficultyConfig(),sel=$('difficultySelect'),hint=$('difficultyHint'),desc=$('introDifficultyDesc'),mapSel=$('mapModeSelect'),leftSel=$('leftAISlots'),rightSel=$('rightAISlots'),workerSel=$('workerDefaultAI'),settlerSel=$('settlerAutoFound'),skirmish=$('skirmishHint');if(sel)sel.value=cfg.key;if(hint)hint.textContent=`${cfg.icon}${cfg.label}：${cfg.desc}`;if(desc)desc.textContent=`${cfg.label}：${cfg.desc}`;if(mapSel)mapSel.value=selectedMapMode;if(leftSel)leftSel.value=String(selectedLeftAI);if(rightSel)rightSel.value=String(selectedRightAI);if(workerSel)workerSel.value=selectedWorkerDefaultAI?'on':'off';if(settlerSel)settlerSel.value=selectedSettlerAutoFound?'on':'off';if(skirmish)skirmish.textContent=`${selectedMapMode==='random'?'随机地图':'默认地图'} · ${1+selectedLeftAI}v${selectedRightAI}${selectedMapMode==='random'&&state?.mapSize?` · ${state.mapSize.width}×${state.mapSize.height}`:''}`;document.querySelectorAll('[data-difficulty]').forEach(b=>b.classList.toggle('active',b.dataset.difficulty===cfg.key));updateAudioUI();}
  ...
  window.__STARFIRE_DEBUG__={
    ...
    setDifficulty,setMapMode,setSkirmishSlots,setWorkerDefaultAI,setSettlerAutoFound, setSpeed:v=>{state.speed=clamp(Number(v)||1,.1,10);}, fortifyPlayer:()=>{const c=state.cities.find(x=>x.team==='player'&&x.capital);c.maxHp=999999;c.hp=999999;}, audio:()=>({ready:gameAudio.ready,muted:gameAudio.muted,volume:gameAudio.volume,contextState:gameAudio.ctx?.state||'none',toneCount:gameAudio.toneCount,lastError:gameAudio.lastError,controls:{settings:!!$('audioDockVolume')}})
  };
