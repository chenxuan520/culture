...
const TUTORIAL_STEPS=[
  ...
  {icon:'🗺️',title:'视野、缩放与快捷键',subtitle:'地图会在鼠标靠近边缘时自动滚动。',target:'.mapBottom',place:'above',task:'把鼠标移到地图边缘滚动视野、按 WASD 或方向键移动视野，或按住空格查看全军名称与血条。',check:'view',body:`鼠标靠近地图边缘会自动平移视野；也可使用 <span class="key">W</span><span class="key">A</span><span class="key">S</span><span class="key">D</span> 或 <span class="key">↑</span><span class="key">↓</span><span class="key">←</span><span class="key">→</span> 平移。鼠标滚轮以指针位置为中心缩放；点击右下角战术雷达可快速跳转。<div class="keys"><span class="key">轻点 A</span> 对鼠标指向执行移动/攻击 <span class="key">G</span> 打开上次生产基地 <span class="key">1-9/0</span> 城市生产快捷键 <span class="key">空格</span> 显示名称血条 <span class="key">P</span> 暂停/继续 <span class="key">F1</span> 打开教程</div>`,tip:'左键拖框现在用于框选单位，不再拖动地图。'},
