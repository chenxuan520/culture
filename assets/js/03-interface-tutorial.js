...
function fmtGain(v){return Number.isInteger(v)?fmt(v):(Math.round(v*10)/10).toLocaleString('zh-CN',{maximumFractionDigits:1});}
function renderTop(){
  for(const k of RESOURCE_KEYS){$(k).textContent=fmt(state.resources[k]);$('g'+k[0].toUpperCase()+k.slice(1)).textContent='+'+fmtGain(state.lastYield[k]||0);}
...
function renderUnitSelection(u){
  ...
  }else html+=`<div class="desc" style="margin-top:10px">选择己方战斗单位后左键此单位，可建立持续追击锁定。</div>`;
...
function productAvailability(city,id){
  ...
  if(!canAfford(d.cost))return{ok:false,reason:'资源不足'};return{ok:true,reason:'可生产'};
}
function groupedQueueItems(queue){
  const groups=[];for(const item of queue){const last=groups[groups.length-1];if(last&&last.id===item.id)last.items.push(item);else groups.push({id:item.id,items:[item]});}return groups;
}
function renderCitySelection(c){
  ...
  let html=`<div class="card"><div class="hero"><div class="heroIcon">${icon}</div><div><h2>${c.name}</h2><p>${c.team==='player'?'城市会自然产生基础资源，并同时维护独立建造队列。':'灰烬军团的核心堡垒，防御与生产能力均弱于玩家。'}</p>${healthBar(c.hp,c.maxHp)}${c.maxShield?healthBar(c.shield,c.maxShield,'linear-gradient(90deg,#7c73ff,#59dcff)'):''}</div></div><div class="badges">${tags}</div><div class="stats">${stat('人口',c.population)}${stat('队列',c.queue.length?`${c.queue.length} 项`:'空')}${stat('基础产出',yieldText(y))}${stat('坐标',`${c.q}, ${c.r}`)}</div>`;
  if(c.team==='player'){
    html+=`<div class="sub">集结点 <span>${c.rallyPoint?`战斗单位完成后前往 ${c.rallyPoint.q},${c.rallyPoint.r}`:'选中城市后左键地图设置'}</span></div><div class="actions"><button class="action ${c.rallyPoint?'':'blocked'} full" data-action="clear-rally" ${c.rallyPoint?'':'data-blocked="当前城市还没有集结点" aria-disabled="true"'}>🎌 清除集结点</button></div>`;
    ...
    groupedQueueItems(c.queue).forEach((group,gi)=>{const item=group.items[0],d=productDef(item.id),count=group.items.length,pct=item.progress/item.time*100;html+=`<div class="queueItem"><div class="qIcon">${d.icon}</div><div><b>${gi===0?'▶ ':''}${d.name}${count>1?` ×${count}`:''}</b><small>${gi===0?`${Math.max(0,item.time-item.progress).toFixed(1)} 秒后完成`:'等待前项完成'} · ${costText(item.cost)}${count>1?` · 共 ${count} 个`:''}</small><div class="progress"><div class="fill" style="width:${gi===0?pct:0}%"></div></div></div><button class="miniBtn" data-cancel="${item.qid}">${count>1?'取消 1 个':'取消'}</button></div>`;});
...
$('selection').addEventListener('click',e=>{
  ...
  const product=e.target.closest('[data-product]');if(product){const c=selectedObject();if(c?.allyAI){toast('盟友城市使用自己的资源和队列，不能用你的资源手动下单。','warn');return;}if(c)queueProduct(c,product.dataset.product,e.shiftKey?5:1);return;}
...
const TUTORIAL_STEPS=[
  ...
  {icon:'🏠',title:'管理城市与建造队列',subtitle:'主城是你生产军队和特殊建筑的核心。',target:'#selection',place:'left',select:'city',task:'在右侧“可生产项目”中点击任意单位或建筑，把它加入建造队列。',check:'queue',body:`我已经替你选中了主城 <b>曙光城</b>。右侧面板上方显示生命、人口和产出；中间是 <em>建造队列</em>；下方是可生产项目。<br><br><b>城市建筑也在这里建</b>，不是派工人去地图上建。点击标着“建筑”的项目，它会进入城市队列，完成后加成自动生效。建造队列没有长度上限，连续相同项目会合并显示成 ×N。按 <span class="key">1-9/0</span> 生产前十个项目，按住 <span class="key">Shift</span> 再按数字会一次尝试加入 5 个单位。点击队列项目右侧的“取消”，会 <b>全额退回资源</b>。`,tip:'前期推荐先造一个作战单位或工人。主城所在格不能再叠加采集设施。'},
  {icon:'🧠',title:'研发树与 AI 助理',subtitle:'科技会解锁兵种、建筑和新的时代。',target:'#techScroll',place:'right',select:'tech',task:'等科研资源够第一项科技后，点击可用科技开始研发，或者勾选“AI 科研助理”让它自动选择。',check:'research',body:`左侧是完整研发树。开局科研为 <b>0</b>，主城默认每次脉冲只提供 <b>+1 科研</b>；农场、矿山和伐木场开局就能建，科技负责强化经济并解锁高级设施。<br><br>想更快研发，就开发星晶实验站或建全息学院。不想逐项管理时，勾选 <b>🤖 AI 科研助理</b>，它会在科研资源允许时自动推进整棵科技树。`,tip:'教程开局默认暂时关闭科研 AI，方便你看清资源门槛；直接开始游戏时仍默认开启。'},
  {icon:'🧭',title:'选择单位并规划路线',subtitle:'单位不是拖动操作，先选中再下命令。',target:'#game',place:'inside-right',select:'warrior',task:'左键确认选中近卫军，再在地图远处的可通行地块点一次左键，看到持续显示的虚线路线。',check:'route',body:`我已经选中并居中了你的近卫军。地图上 <b>左键</b>点选城市、单位或资源地块；也可以 <b>按住左键拖框</b>一次选中多个己方单位。选中己方单位后，在远处空地再点一次 <em>左键</em>，系统会用六边形寻路规划完整路线。<br><br><b>右键选中单位时取消选择；没选中单位时按住右键拖动地图。</b> 单位会按固定时钟周期逐格前进，虚线会一直保留到抵达目的地。`,tip:'水域和山脉通常不可通行；基洛夫飞艇等飞行单位可以无视地形。'},
  {icon:'🎯',title:'锁敌、追击与驻守',subtitle:'战斗单位可自动作战，也可以固定守一个格子。',target:'.mission',place:'below',select:'warrior',body:`己方战斗单位周围 <em>3 格</em>内出现敌人时，默认会自动锁定并攻击。你也可以先选中己方作战单位，再对敌军点 <b>左键</b>：单位会显示 🎯 标记，持续追赶，并在合适射程停下开火。<br><br>如果你想让部队站在当前格子不要主动追击，点右侧 <b>驻守当前格</b>。驻守后单位仍会攻击自己射程内的敌人，但不会追出当前格；你手动左键移动或左键敌人时，会自动取消驻守。`,tip:'多个不同职责的兵种靠近会有兵种配合加成；多辆光棱坦克靠近会获得组网加成。'},
  {icon:'👷',title:'工人 AI 与资源开发',subtitle:'工人会因地制宜自动建设，最多完成五项工程。',target:'#selection',place:'left',select:'worker',task:'在右侧面板勾选“工人 AI 模式”。',check:'workerAI',body:`我已经选中了工人。开启右侧的 <b>工人 AI 模式</b>后，它会自动寻找附近未开发资源，并按地形建设农场、矿山或永续伐木场；星晶实验站、萃取井、日冕电站等高级设施需要对应科技。<br><br>每名工人有 <em>5 次建设次数</em>，完成第五项工程后退役。工人靠近受伤的友军、城市或设施时还会自动维修。`,tip:'也可以直接点击一个资源地块，再使用“派最近工人开发”进行手动指派。'},
  ...
  {icon:'🗺️',title:'视野、缩放与快捷键',subtitle:'视野移动由键盘和右键拖图控制。',target:'.mapBottom',place:'above',task:'用 WASD 或方向键移动视野，右键拖动地图，或按住空格查看全军名称与血条。',check:'view',body:`使用 <span class="key">W</span><span class="key">A</span><span class="key">S</span><span class="key">D</span> 或 <span class="key">↑</span><span class="key">↓</span><span class="key">←</span><span class="key">→</span> 平移视野；没有选中己方单位时，也可以按住右键拖动地图。鼠标滚轮以指针位置为中心缩放；点击右下角战术雷达可快速跳转。<div class="keys"><span class="key">H</span> 打开上次生产基地 <span class="key">1-9/0</span> 城市生产快捷键 <span class="key">Shift+1-9/0</span> 一次尝试生产 5 个 <span class="key">空格</span> 显示名称血条 <span class="key">P</span> 暂停/继续 <span class="key">F1</span> 打开教程</div>`,tip:'左键拖框用于框选单位；右键拖图只在没有选中己方单位时生效。'},
...
function demoTutorialStep(){
  ...
  else if(i===3){tutorial.flags.researchChoiceTouched=true;state.aiTech=true;const first=TECHS.filter(availableTech).sort((a,b)=>a.cost-b.cost)[0];if(first)state.resources.science=Math.max(state.resources.science,first.cost);if(!state.research)chooseAIResearch();toast('▶ 演示：已补足第一项科技所需科研，并开启 AI 科研助理。','good');}
