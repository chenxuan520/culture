...
(() => {
  ...
  let selectedDifficulty='medium',selectedMapMode='default',selectedLeftAI=0,selectedRightAI=1,selectedPlayerSide='left',selectedEnemyFaction='rbp',selectedWorkerDefaultAI=false,selectedSettlerAutoFound=false;
  try{
    ...
    const enemyFaction=localStorage.getItem('starfireEnemyFaction'),oldFaction=localStorage.getItem('starfirePlayerFaction');if(enemyFaction==='bgy'||enemyFaction==='rbp')selectedEnemyFaction=enemyFaction;else if(oldFaction==='rbp')selectedEnemyFaction='bgy';else if(oldFaction==='bgy'||['blue','green','yellow','verdant','auric'].includes(oldFaction))selectedEnemyFaction='rbp';
  ...
  const TEAM_FACTIONS={bgy:['blue','green','yellow'],rbp:['red','black','purple']};
  function teamFactions(team){return TEAM_FACTIONS[team]||TEAM_FACTIONS.bgy;}
  function opposingTeam(team){return team==='rbp'?'bgy':'rbp';}
  ...
  freshState=function(started=false,difficulty=selectedDifficulty,mapMode=selectedMapMode,leftAI=selectedLeftAI,rightAI=selectedRightAI,playerSide=selectedPlayerSide,enemyFaction=selectedEnemyFaction){
    leftAI=clamp(Number(leftAI)||0,0,2);rightAI=clamp(Number(rightAI)||1,1,3);
    playerSide=playerSide==='right'?'right':'left';enemyFaction=normalizeEnemyFaction(enemyFaction);
    ...
    const friendlyOrder=teamFactions(opposingTeam(enemyFaction)),enemyOrder=teamFactions(enemyFaction);
    ...
    const s={started,paused:false,gameOver:false,speed:1,simTime:0,pulseTimer:0,enemySpawnTimer:0,enemyThink:0,difficulty,
      ...
      mapMode:currentMapConfig.mode,mapSeed:currentMapConfig.seed,mapSize:{width:mapWidth(),height:mapHeight()},leftAI,rightAI,playerSide,enemyFaction,playerMapSide,enemyMapSide,
  ...
  function renderSettings(){const cfg=difficultyConfig(),sel=$('difficultySelect'),hint=$('difficultyHint'),desc=$('introDifficultyDesc'),mapSel=$('mapModeSelect'),enemyFactionSel=$('enemyFactionSelect'),sideSel=$('playerSideSelect'),leftSel=$('leftAISlots'),rightSel=$('rightAISlots'),workerSel=$('workerDefaultAI'),settlerSel=$('settlerAutoFound'),skirmish=$('skirmishHint');if(sel)sel.value=cfg.key;if(hint)hint.textContent=`${cfg.icon}${cfg.label}：${cfg.desc}`;if(desc)desc.textContent=`${cfg.label}：${cfg.desc}`;if(mapSel)mapSel.value=selectedMapMode;if(enemyFactionSel)enemyFactionSel.value=selectedEnemyFaction;if(sideSel)sideSel.value=selectedPlayerSide;if(leftSel)leftSel.value=String(selectedLeftAI);if(rightSel)rightSel.value=String(selectedRightAI);if(workerSel)workerSel.value=selectedWorkerDefaultAI?'on':'off';if(settlerSel)settlerSel.value=selectedSettlerAutoFound?'on':'off';if(skirmish)skirmish.textContent=`${selectedMapMode==='random'?'随机地图':'默认地图'} · 敌方${factionLabel()} · ${sideLabel()} · ${1+selectedLeftAI}v${selectedRightAI}${selectedMapMode==='random'&&state?.mapSize?` · ${state.mapSize.width}×${state.mapSize.height}`:''}`;document.querySelectorAll('[data-difficulty]').forEach(b=>b.classList.toggle('active',b.dataset.difficulty===cfg.key));updateAudioUI();}
  ...
  function improvementColor(improvement){
    ...
    return FACTION_COLORS[teamFactions(playerTeam())[0]];
  }
  ...
  // ===== 难度选择、重开、启动与 WASD =====
  function restartWithSelectedConfig(started=false){const oldSpeed=state?.speed||1;state=freshState(started,selectedDifficulty,selectedMapMode,selectedLeftAI,selectedRightAI,selectedPlayerSide,selectedEnemyFaction);state.speed=oldSpeed;resizeCanvases();state.lastYield=calculateYield();state.logs=[];addLog('🏠 曙光城开始自然产生基础资源。','good');addLog(`${selectedMapMode==='random'?'🎲 随机地图':'⬡ 默认地图'} · 敌方${factionLabel()} · ${sideLabel()} · ${1+selectedLeftAI}v${selectedRightAI} 对战配置已载入。`,'good');addLog(`${difficultyConfig().icon} 灰烬文明按「${difficultyConfig().label}」规则开始独立发展。`,'warn');clampCamera();renderPanels();renderSettings();}
  ...
  function setEnemyFaction(faction,announce=true){faction=normalizeEnemyFaction(faction);const old=selectedEnemyFaction;selectedEnemyFaction=faction;try{localStorage.setItem('starfireEnemyFaction',faction);}catch{}if(!state.started)restartWithSelectedConfig(false);renderSettings();if(announce&&old!==faction)toast(`敌方阵营已切换为${factionLabel(faction)}。`,'good');}
  ...
  $('enemyFactionSelect')?.addEventListener('change',e=>{tutorial.flags.settingsTouched=true;setEnemyFaction(e.target.value,true);updateTutorialTask();});
